...
- **App Store**: **1.5 在售**；**1.6 / build 266（`1c77342`，长按菜单 itemId 溯源 + 公众号真验证/取号指引 + 提示词管理修复）已提审 `WAITING_FOR_REVIEW`**（2026-07-19，AFTER_APPROVAL 过审即发布）。**1.6 提审踩坑（已解决）**：ASC 上有个 READY_FOR_REVIEW 的提交草稿装着**订阅组条目**（monthly_19_9 首次审核需搭版本），fastlane 拒绝（items not managed by fastlane）；且订阅本体是**空壳**（0 本地化/0 价格/无审核截图 → MISSING_METADATA），组无法过闸。处置：API 把订阅组条目从提交里摘除（DELETE reviewSubmissionItems）、把 fastlane 已备好的 1.6 版本加进同一提交（POST reviewSubmissionItems）、PATCH submitted:true。**订阅要上架还欠**：subscriptionLocalizations（zh-Hans 名称+描述）、订阅价格（¥19.9 pricePoint）、审核截图（付费墙 UI 实拍）、availability，补齐后随 1.7 一起提审（首个订阅必须搭版本）。ASC 隐私标签用户已手动更新（2026-07-15：标识符→用户 ID + 使用数据→产品交互，关联身份、用途=分析、不追踪）。提审流程：project.yml MARKETING_VERSION 垫到目标版本 + 更新双语 `fastlane/metadata/*/release_notes.txt` → push main（不带 `[tf]`，只验编译）→ dispatch `appstore`（`fastlane release skip_build:true` 复用 TestFlight 已处理 build）→ 打 `release/<v>` tag 推远端（CI 里 lane 打的 tag 不会推回来）。(Resubmit = cancel review submission via ASC API `PATCH reviewSubmissions/{id} canceled:true`, then dispatch `appstore`.)
